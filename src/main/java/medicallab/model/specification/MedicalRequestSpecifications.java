...
import medicallab.model.MedicalRequest_;
import medicallab.model.Request;
import medicallab.model.Request_;
import medicallab.model.User;
import medicallab.model.User_;

public class MedicalRequestSpecifications {

	public static Specification<Request> searchAndFilterForPatientFullnameLike(String searchText,
			String testTypeListAsString, String requestStatusListAsString) {
		return (root, query, builder) -> {
			return builder.and(patientFullnameLike(searchText).toPredicate(root, query, builder),
	...
	public static Specification<Request> searchAndFilterForPatientIdLike(String searchText, String testTypeListAsString,
			String requestStatusListAsString) {
		return (root, query, builder) -> {
			return builder.and(patientIdLike(searchText).toPredicate(root, query, builder),
					testTypeIn(testTypeListAsString).toPredicate(root, query, builder),
					requestStatusIn(requestStatusListAsString).toPredicate(root, query, builder));
		};
	}

	public static Specification<Request> searchAndFilterForDoctorFullnameLike(String searchText,
			String testTypeListAsString, String requestStatusListAsString) {
		return (root, query, builder) -> {
			return builder.and(doctorFullnameLike(searchText).toPredicate(root, query, builder),
					testTypeIn(testTypeListAsString).toPredicate(root, query, builder),
					requestStatusIn(requestStatusListAsString).toPredicate(root, query, builder));
		};
	}

	public static Specification<Request> searchAndFilterForRequestCodeLike(String searchText,
			String testTypeListAsString, String requestStatusListAsString) {
		return (root, query, builder) -> {
			return builder.and(requestCodeLike(searchText).toPredicate(root, query, builder),
					testTypeIn(testTypeListAsString).toPredicate(root, query, builder),
					requestStatusIn(requestStatusListAsString).toPredicate(root, query, builder));
		};
	}
	...
	public static Specification<MedicalRequest> doctorIs(User doctor) {
		return (root, query, builder) -> {
			Expression<String> doctorExpression = root.get("doctor_id");
			
			return builder.like(doctorExpression, doctor.getId().toString());
		};
	}
